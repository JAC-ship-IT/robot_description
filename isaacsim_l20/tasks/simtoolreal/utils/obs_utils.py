...
PALM_CENTER_OFFSET: tuple[float, float, float] = (0.0, -0.03, 0.18)
# Shift fingertip body origins to the approximate pad centers.
FINGERTIP_OFFSET: tuple[float, float, float] = (0.03, 0, -0.001)
THUMB_OFFSET: tuple[float, float, float] = (0.042, 0.0, -0.01)
...
def build_observations(env) -> dict[str, torch.Tensor]:
    ...
    ft_pos_w = torch.zeros_like(ft_body_pos_w)
    ft_pos_w[:, 1:] = _apply_local_offset(
        ft_body_pos_w[:, 1:],
        ft_body_rot_w[:, 1:],
        FINGERTIP_OFFSET,
        (env.num_envs, NUM_FINGERTIPS - 1),
    )
    ft_pos_w[:, 0] = _apply_local_offset(
        ft_body_pos_w[:, 0],
        ft_body_rot_w[:, 0],
        THUMB_OFFSET,
        (env.num_envs,),

...
class EnvStatsAlgoObserver(AlgoObserver):
        ...
        if isinstance(log_dict, dict):
            for key, value in log_dict.items():
                if isinstance(value, torch.Tensor) and value.ndim > 0:
                    self.direct_info[f"log/{key}"] = _mean_float(value)
                elif _is_scalar(value):
                    self.direct_info[f"log/{key}"] = _as_float(value)
    ...
    def gather_env_metrics(self) -> dict[str, float]:
        """Collect env stats for TensorBoard / wandb without mutating observer state."""
        metrics: dict[str, float] = {}
        if self.new_finished_episodes:
            for key, values in self.episode_cumulative_avg.items():
                if values:
                    metrics[f"episode_cumulative/{key}"] = float(np.mean(values))
                    metrics[f"episode_cumulative_min/{key}_min"] = float(np.min(values))
                    metrics[f"episode_cumulative_max/{key}_max"] = float(np.max(values))
            for key, values in self.episode_final_avg.items():
                if values:
                    metrics[f"episode_final/{key}"] = float(np.mean(values))
    ...
    def after_print_stats(self, frame, epoch_num, total_time):
        ...
        if self.new_finished_episodes:
            for key, values in self.episode_cumulative_avg.items():
                if values:
                    self.writer.add_scalar(f"episode_cumulative/{key}", metrics[f"episode_cumulative/{key}"], frame)
                    self.writer.add_scalar(
                        f"episode_cumulative_min/{key}_min",
                        metrics[f"episode_cumulative_min/{key}_min"],
                        frame,
                    )
                    self.writer.add_scalar(
                        f"episode_cumulative_max/{key}_max",
                        metrics[f"episode_cumulative_max/{key}_max"],
                        frame,
                    )
            for key, values in self.episode_final_avg.items():
                if values:
                    self.writer.add_scalar(f"episode_final/{key}", metrics[f"episode_final/{key}"], frame)
            self.new_finished_episodes = False

        for key, value in self.direct_info.items():
            self.writer.add_scalar(key, metrics[key], frame)

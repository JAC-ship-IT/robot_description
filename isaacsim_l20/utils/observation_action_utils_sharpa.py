...
assert len(JOINT_NAMES_ISAACGYM) == 27, (
    f"len(JOINT_NAMES_ISAACGYM): {len(JOINT_NAMES_ISAACGYM)}, expected: 27"
)
...
assert Q_LOWER_LIMITS_np.shape == (27,), (
    f"Q_LOWER_LIMITS_np.shape: {Q_LOWER_LIMITS_np.shape}, expected: (29,)"
)
assert Q_UPPER_LIMITS_np.shape == (27,), (
    f"Q_UPPER_LIMITS_np.shape: {Q_UPPER_LIMITS_np.shape}, expected: (29,)"
)

Q_LOWER_LIMITS_restricted_np = Q_LOWER_LIMITS_np.copy()
Q_LOWER_LIMITS_restricted_np[:6] += np.deg2rad(10.0)

Q_UPPER_LIMITS_restricted_np = Q_UPPER_LIMITS_np.copy()
Q_UPPER_LIMITS_restricted_np[:6] -= np.deg2rad(10.0)
assert Q_LOWER_LIMITS_restricted_np.shape == (27,), (
    f"Q_LOWER_LIMITS_restricted_np.shape: {Q_LOWER_LIMITS_restricted_np.shape}, expected: (27,)"
)
assert Q_UPPER_LIMITS_restricted_np.shape == (27,), (
    f"Q_UPPER_LIMITS_restricted_np.shape: {Q_UPPER_LIMITS_restricted_np.shape}, expected: (27,)"
)
...
T_W_R_np = np.eye(4)
T_W_R_np[:3, 3] = np.array([0.0, 0.0, 0.0])

PALM_OFFSET_np = np.array([0.0, -0.03, 0.18])
...
def compute_fk_dict(
    ...
    N = q.shape[0]
    assert q.shape == (N, 27), f"q.shape: {q.shape}, expected: (N, 27)"
...
def compute_joint_pos_targets(
    ...
    cur_targets[:, 6:27] = scale(
        actions[:, 6:27],
        q_lower_limits[6:27],
        q_upper_limits[6:27],
    )
    cur_targets[:, 6:27] = (
        hand_moving_average * cur_targets[:, 6:27]
        + (1.0 - hand_moving_average) * prev_targets[:, 6:27]
    )
    cur_targets[:, 6:27] = tensor_clamp(
        cur_targets[:, 6:27],
        q_lower_limits[6:27],
        q_upper_limits[6:27],
    )
    ...
    # arm
    cur_targets[:, :6] = (
        prev_targets[:, :6] + hand_dof_speed_scale * dt * actions[:, :6]
    )
    cur_targets[:, :6] = tensor_clamp(
        cur_targets[:, :6],
        q_lower_limits[:6],
        q_upper_limits[:6],
    )
    cur_targets[:, :6] = (
        arm_moving_average * cur_targets[:, :6]
        + (1.0 - arm_moving_average) * prev_targets[:, :6]
    )
    return cur_targets
